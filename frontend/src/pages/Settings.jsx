...
export default function Settings() {
  const [conn, setConn] = useState({ connected: false, host: "", port: "" });
  const [settings, setSettings] = useState({
    ...
    neighbours_enabled: false,
    neighbours_check_hours: 6,
    clock_check_enabled: false,
    clock_check_days: 24,
    firmware_get_enabled: false,
    firmware_get_days: 7,
    map_path_max_km: 300,
    node_id_chars: 2,
    ntfy_topic: "",
  ...
  useEffect(() => {
    const loadConn = () => {
      fetch("/api/connection")
        .then((r) => r.json())
        .then((d) => {
          setConn({ connected: d.connected, host: d.host, port: d.port });
        })
        .catch(() => {});
    };
    loadConn();
    const interval = setInterval(loadConn, 5000);
    ...
    fetch("/api/settings")
      ...
      .then((s) => {
        setSettings({
          ...
          stagger_delay_seconds: s.stagger_delay_seconds || 30,
          low_battery_percent: s.low_battery_percent || 20,
          neighbours_enabled: s.neighbours_enabled || false,
          neighbours_check_hours: s.neighbours_check_hours || 6,
          clock_check_enabled: s.clock_check_enabled || false,
          clock_check_days: s.clock_check_days || 24,
          firmware_get_enabled: s.firmware_get_enabled || false,
          firmware_get_days: s.firmware_get_days || 7,
          map_path_max_km: s.map_path_max_km || 300,
          node_id_chars: s.node_id_chars || 2,
          ntfy_topic: s.ntfy_topic || "",
          ntfy_server: s.ntfy_server || "https://ntfy.sh",
  ...
  return (
    <div className={styles.settingsPageBody}>
      <div className={styles.settingsSection}>
        ...
        <div className={styles.connStatusRow}>
          <span
            className={`status-dot ${conn.connected ? "online" : "offline"}`}
          ></span>
          <span className={styles.connStatusText}>
            {conn.connected ? (
              <>
                Connected to{" "}
                <strong>
                ...
                >
                  ...
                  <div className={styles.settingsField}>
                    ...
                    <input
                      type="text"
                      value={r.path || ""}
                      onInput={(e) => updateRepeater(i, "path", e.target.value)}
                    />
                  </div>
                  <div className={styles.settingsField}>
                    ...
                    <input
                      type="text"
                      value={r.lat || ""}
                      onInput={(e) => updateRepeater(i, "lat", e.target.value)}
                    />
                  </div>
                  <div className={styles.settingsField}>
                    ...
                    <input
                      type="text"
                      value={r.lon || ""}
                      onInput={(e) => updateRepeater(i, "lon", e.target.value)}
                    />
                  </div>
                </div>
                <button
