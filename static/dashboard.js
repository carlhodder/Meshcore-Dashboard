...
function pingRepeater(pubkey, btn) {
  ...
  fetch("/api/ping/" + encodeURIComponent(pubkey), { method: "POST" })
    ...
    .then(function (result) {
      ...
      var b = document.querySelector(
        '[data-pubkey="' + pubkey + '"] [data-action="ping"]',
      );
      if (b)
        b.className = "card-ping-btn " + (result.ok ? "ping-ok" : "ping-fail");
    })
    .catch(function () {
...
function _startPingCountdown(pubkey) {
  ...
  function tick() {
    var remaining = Math.max(
      0,
      Math.ceil(window._pingCooldowns[pubkey] - Date.now() / 1000),
    );
    var btn = document.querySelector(
      '[data-pubkey="' + pubkey + '"] [data-action="ping"]',
    );
  ...
  fetch(url)
    ...
    .then(function (logs) {
      if (!logs || logs.length === 0) {
        tbody.innerHTML =
          '<tr><td colspan="4" class="logs-empty">No log entries found for this time range.</td></tr>';
        return;
      }
      ...
      logs.forEach(function (entry) {
        ...
        tdTime.textContent = new Date(entry.timestamp * 1000).toLocaleString(
          [],
          {
            month: "short",
            day: "numeric",
    ...
    .catch(function (err) {
      tbody.innerHTML =
        '<tr><td colspan="4" class="logs-empty">Failed to load logs: ' +
...
document.addEventListener("DOMContentLoaded", function () {
  ...
  fetch("/api/settings")
    ...
    .then(function (s) {
      ...
      if (!window._cardOrder || window._cardOrder.length === 0) {
        window._cardOrder = (s.repeaters || []).map(function (r) {
          return r.pubkey;
        });
      }
        ...
        if (d.connected) {
          ...
          if (d.battery_mv && d.battery_mv > 0) {
            ...
            batStr =
              ...
              pct +
              "% (" +
              (d.battery_mv / 1000).toFixed(2) +
              "V)</span>";
          }
          text.innerHTML =
            "Node: " + escapeHtml(d.host || "connected") + batStr;
          if (btn) {
            btn.style.display = "";
            btn.textContent =
              d.polling_enabled === false
                ...
                : "⏸ Pause Polling";
            btn.style.color =
              d.polling_enabled === false ? "#f59e0b" : "#94a3b8";
            btn.style.borderColor =
              d.polling_enabled === false ? "#f59e0b" : "#334155";
          }
        } else {
          ...
          if (d.last_connected && d.last_connected > 0) {
            ...
            if (ago < 60) lastStr = " &middot; last seen " + ago + "s ago";
              ...
              lastStr = " &middot; last seen " + Math.floor(ago / 60) + "m ago";
            else if (ago < 86400)
              lastStr =
                " &middot; last seen " + Math.floor(ago / 3600) + "h ago";
            else
              lastStr =
                " &middot; last seen " +
  ...
  function checkUnreadMessages() {
    ...
    fetch("/api/messages?hours=48&limit=1")
      ...
      .then(function (msgs) {
        ...
        if (!badge) return;
        badge.style.display =
          msgs.length > 0 && msgs[0].timestamp > lastSeen ? "" : "none";
      })
      .catch(function () {});
  }
  checkUnreadMessages();
